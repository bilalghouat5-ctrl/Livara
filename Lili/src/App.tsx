...
// New UI/UX Pro Max features
import { SplashScreen } from './components/SplashScreen';
import { Haptic } from './components/HapticEngine';
import { useNotifications, NotifToast, NotifPanel, NotifBell } from './components/NotificationCenter';
import { FullPageSkeleton } from './components/SkeletonComponents';
...
function DashboardLayout() {
  const [splashDone, setSplashDone] = useState(() => {
    return sessionStorage.getItem('lacasa_splash_shown') === '1';
  });
  ...
  const handleTabChange = (tab: 'explore' | 'favorites' | 'trips' | 'messages' | 'profile') => {
    ...
    setTimeout(() => setIsTabLoading(false), 480);
    ...
    <div className="w-full h-full flex flex-col justify-between overflow-hidden relative bg-white text-zinc-900">
      ...
      {!splashDone && (
        <SplashScreen onFinished={() => {
          sessionStorage.setItem('lacasa_splash_shown', '1');
          setSplashDone(true);
          Haptic.success();
        }} />
      )}
      ...
      <AnimatePresence>
        {notif.toastQueue.length > 0 && (
          <NotifToast
            notification={notif.toastQueue[0]}
      ...
      {/* NOTIFICATION PANEL */}
      <NotifPanel
        isOpen={notif.isPanelOpen}
        onClose={() => notif.setIsPanelOpen(false)}
        notifications={notif.notifications}
        onMarkAllRead={notif.markAllRead}
        onDismiss={notif.dismissNotif}
      />
      ...
      <div className="absolute top-14 left-3 z-30">
        <NotifBell
          ...
          onClick={() => notif.setIsPanelOpen(true)}
        />
      </div>
