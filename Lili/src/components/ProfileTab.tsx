...
export const ProfileTab: React.FC = () => {
  const { userProfile, updateUserProfile, bookings, favorites, triggerModeTransition, isHostRegistered, setIsHostRegistered } = useApp();
      ...
      <AnimatePresence mode="wait">
        {!activeSubView ? (
          ...
          >
            ...
            <AnimatePresence>
              {scrollVisible && isHostRegistered && (
                <motion.div
          ...
          >
            ...
            >
              ...
              <div className="px-6 pb-8 space-y-6 overflow-y-auto no-scrollbar">
                ...
                {/* "اهتماماتي" Section exactly matching Image 5 */}
                <div className="pt-6 space-y-3 text-right">
                  <h4 className="text-sm font-black text-zinc-950">اهتماماتي</h4>
                  <p className="text-[11.5px] text-zinc-500 font-bold leading-relaxed text-right">
                    يمكنك العثور على قواسم مشتركة بينك وبين الضيوف والمضيفين الآخرين من خلال إضافة اهتماماتك إلى ملفك الشخصي.
                  </p>

                  {/* 3 bubbles + plus and additions centered perfectly */}
                  <div className="flex flex-wrap gap-4.5 justify-center items-center pt-3 pb-1 w-full mx-auto">
                    {interests.map((interest, idx) => (
                      <span 
                        key={idx} 
                        className="inline-flex items-center gap-1.5 bg-zinc-50 hover:bg-zinc-100 text-zinc-800 border border-zinc-200 rounded-full px-4 py-2.5 text-xs font-black cursor-pointer transition-colors shadow-xs"
                        onClick={() => {
                          setInterests(interests.filter((_, i) => i !== idx));
                        }}
                      >
                        <span>{interest}</span>
                        <X className="w-3.5 h-3.5 stroke-[2.5px] text-zinc-500 hover:text-rose-500" />
                      </span>
                    ))}

                    {/* Empty dashed bubble sockets from screenshot 5 */}
                    {Array.from({ length: Math.max(0, 3 - interests.length) }).map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => setShowInterestsDialog(true)}
                        className="w-14 h-14 border-2 border-dashed border-zinc-300 hover:border-[#ff385c] rounded-full flex items-center justify-center text-zinc-400 hover:text-[#ff385c] hover:bg-rose-50/10 cursor-pointer active:scale-90 transition-all shrink-0 shadow-inner"
                        title="إضافة اهتمام جديد"
                      >
                  ...
                  <button
                    type="button"
                    onClick={() => setShowInterestsDialog(true)}
                    className="w-full text-center py-3.5 bg-zinc-50 hover:bg-zinc-100 border border-zinc-200 text-zinc-900 rounded-2xl text-xs font-black transition-all cursor-pointer active:scale-95"
                  >
                    أضف الاهتمامات
                  </button>
                </div>
