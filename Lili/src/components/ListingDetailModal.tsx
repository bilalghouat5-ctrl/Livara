...
export const ListingDetailModal: React.FC<ListingDetailModalProps> = ({ listing, onClose }) => {
  ...
  // Premium Haptic Feedback Simulated and Real Vibration Engine
  const triggerHaptic = (type: 'light' | 'medium' | 'heavy' | 'notification' = 'light') => {
    if (typeof window !== 'undefined' && window.navigator && window.navigator.vibrate) {
      try {
        ...
        // Suppress errors about vibration context
      }
    }
  };

  // Tap-and-Slide Swipe Back Gesture definition for fluid native mobile experiences
  const [swipeStartX, setSwipeStartX] = useState<number | null>(null);
  const [swipeStartY, setSwipeStartY] = useState<number | null>(null);

  const handleTouchStart = (e: React.TouchEvent) => {
    const target = e.target as HTMLElement;
    // Exclude image carousels, buttons, inputs or options
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Validate horizontal swipe over 110px threshold, with a small vertical alignment skew
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setBannerState('docked');
    }, 2500); // 2.5 seconds floating, then docks
  ...
  if (isServiceCategory) {
    ...
    return (
      <div 
        ...
        className="fixed inset-0 z-50 flex items-start sm:items-center justify-center bg-white sm:bg-zinc-950/60 sm:backdrop-blur-md p-0 sm:p-4 overflow-y-auto"
      >
        <motion.div 
          ...
          className="relative bg-white w-full sm:max-w-2xl min-h-screen sm:min-h-0 sm:max-h-[94vh] sm:rounded-[36px] overflow-y-auto shadow-none sm:shadow-2xl scrollbar-none border-0 sm:border border-zinc-200/40 pb-32 sheet-enter gpu"
        ...
        >
          ...
          <AnimatePresence>
            {copiedLink && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                className="absolute top-16 left-1/2 -translate-x-1/2 z-40 bg-zinc-900 text-white py-2.5 px-6 rounded-full text-[11px] font-black shadow-lg border border-zinc-800"
              >
          ...
          {/* Hero Banner Image */}
          <div className="relative h-[44vh] sm:h-96 w-full bg-zinc-100 overflow-hidden no-swipe-back">
            <motion.img 
            ...
            {/* Float Floating Action Header Row with pixel-perfect icons from screenshots */}
            <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between" dir="rtl">
              {/* Back to listings button on the right edge */}
              <button 
                onClick={onClose}
                className="bg-white hover:bg-zinc-50 border-0 text-zinc-900 p-2.5 rounded-full shadow-md transition-all flex items-center justify-center cursor-pointer haptic-tap"
                type="button"
                id="photographer-back-button"
              >
                <ArrowRight className="w-5 h-5 text-zinc-900 stroke-[2.2]" />
              </button>
              ...
              <div className="flex gap-2.5">
                <button 
                  ...
                  className="bg-white hover:bg-zinc-50 border-0 text-zinc-700 p-2.5 rounded-full shadow-md transition-all flex items-center justify-center cursor-pointer haptic-tap"
                  type="button"
                >
                  <Upload className="w-5 h-5 text-zinc-800 stroke-[2.2]" />
                </button>
                <button 
                  onClick={() => toggleFavorite(listing.id)}
                  className="bg-white hover:bg-zinc-50 border-0 text-rose-500 p-2.5 rounded-full shadow-md transition-all flex items-center justify-center cursor-pointer haptic-tap"
                  type="button"
                >
                  <Heart className={`w-5 h-5 ${isFavorite ? 'fill-rose-500 text-rose-500' : 'text-zinc-700 stroke-[2.2]'}`} />
                </button>
              </div>
            </div>
          </div>

          {/* White Card Pull-Up overlaying the image bottom part */}
          <div className="bg-white rounded-t-[36px] -mt-10 relative z-10 pt-12 px-5 sm:px-6 pb-20 text-right">
            ...
            {/* Translated Marker */}
            <div className="flex items-center justify-center gap-1.5 text-[11px] text-zinc-500 font-bold mt-2 select-none">
              <span className="text-zinc-500 hover:text-zinc-700 cursor-pointer">تم الترجمة آليًا</span>
            ...
            <div className="space-y-4 max-w-md mx-auto">
              {config.packages.map((pkg, idx) => (
                <div key={idx} className="bg-white border border-zinc-150 rounded-[24px] p-4 flex flex-row items-center gap-4 text-right hover:border-zinc-300 transition-all shadow-xs">
                  ...
                  {/* Left Thumbnail */}
                  <div className="w-[84px] h-[84px] rounded-2xl overflow-hidden shrink-0 bg-zinc-100 shadow-xs">
                    <img 
                      src={pkg.image} 
                      alt={pkg.title} 
                      className="w-full h-full object-cover"
            ...
            {/* "نماذج أعمالي (البورتفوليو)" Section */}
            <div className="space-y-4 max-w-md mx-auto">
              <h2 className="text-base font-black text-zinc-950 text-right pr-0.5">
                نماذج أعمالنا (معرض الصور)
              </h2>
              {/* Multi-photo grid Layout */}
              <div className="grid grid-cols-12 gap-3 select-none">
                {/* Right Column (Col Span 7 of 12) - Large Portrait Image */}
                <div className="col-span-7 h-64 rounded-3xl overflow-hidden bg-zinc-100 shadow-xs">
                  <img 
                    src={gridImages[0]} 
                    alt="Portfolio 1" 
                ...
                {/* Left Column (Col Span 5 of 12) - Vertical list of 2 smaller images */}
                <div className="col-span-5 flex flex-col gap-3 h-64">
                  <div className="flex-1 rounded-[20px] overflow-hidden bg-zinc-100 shadow-xs">
                    <img 
                      src={gridImages[1]} 
                      alt="Portfolio 2" 
                      className="w-full h-full object-cover hover:scale-105 transition-transform duration-500"
                  ...
                  <div className="flex-1 rounded-[20px] overflow-hidden bg-zinc-100 relative shadow-xs">
                    <img 
                      src={gridImages[2]} 
                      alt="Portfolio 3" 
                      className="w-full h-full object-cover hover:scale-105 transition-transform duration-500"
                      referrerPolicy="no-referrer"
                    />
                    {/* Folder overlapping indicator */}
                    <div className="absolute bottom-2 left-2 bg-white/80 backdrop-blur-xs p-1.5 rounded-lg shadow-sm">
                      <svg className="w-3.5 h-3.5 text-zinc-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
            ...
            <div className="space-y-3 mt-8 text-right max-w-md mx-auto">
              ...
              <p className="text-zinc-500 text-[11.5px] font-semibold leading-relaxed pr-0.5 max-w-sm">
                نأتي إلى موقع الضيوف في نطاق المنطقة الموضحة على الخريطة. يمكنك مراسلتنا لحجز الخدمة في موقع مختلف.
              </p>
              {/* Map UI box */}
              <div className="w-full h-40 rounded-[28px] overflow-hidden mt-3 relative border border-zinc-150 shadow-xs">
                <img 
                  src="https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&w=500&q=80"
                  alt={`${listing.location.ar} Map location`}
                  className="w-full h-full object-cover brightness-[0.98] contrast-[1.02]"
                />
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-18 h-18 rounded-full bg-rose-500/10 border-2 border-rose-500/60 animate-pulse flex items-center justify-center">
                    <div className="w-3 h-3 rounded-full bg-rose-600 border border-white shadow-md" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6 mt-9 text-right max-w-md mx-auto border-t border-zinc-100 pt-7 font-sans">
              ...
              <div className="bg-white border border-zinc-150 rounded-[28px] p-5 space-y-4 shadow-xs">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-extrabold text-zinc-400 select-none">منذ 3 أيام</span>
          ...
          <div className="fixed sm:absolute bottom-0 left-0 right-0 z-45 max-w-2xl mx-auto w-full flex flex-col pointer-events-none" dir="rtl">
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Hero Image Slider - Taller height on mobile for perfect matching */}
        <div className="relative h-[44vh] sm:h-96 w-full bg-zinc-100 overflow-hidden no-swipe-back">
          <motion.div
            key={currentImgIndex}
          ...
          {/* Dots Indicator inside image / Pagination count (1 / 6) on bottom-left */}
          <div className="absolute bottom-14 left-4 z-10 bg-zinc-950/70 text-white text-[10px] font-black py-1 px-3 rounded-md animate-none" dir="ltr">
            {currentImgIndex + 1} / {listing.images.length}
          </div>
        </div>

        {/* Sticky Header Actions - Placed after slider and configured with z-40 and no click blocking pointer events */}
        <div className="absolute top-4 left-4 right-4 z-40 flex justify-between items-center" dir="rtl">
          {/* Close/Back Button - White Circle pointing right (exit back in Arabic RTL layout) */}
          <button 
            onClick={onClose}
            className="bg-white hover:bg-zinc-50 border-0 text-zinc-900 p-2.5 rounded-full shadow-md transition-all flex items-center justify-center cursor-pointer haptic-tap"
          ...
          <div className="flex gap-2">
            {/* Share circle button - iOS style upload share icon */}
            <button 
              onClick={handleShareClick}
              className="bg-white hover:bg-zinc-50 border-0 text-zinc-700 p-2.5 rounded-full shadow-md transition-all flex items-center justify-center cursor-pointer haptic-tap"
              ...
              type="button"
            >
              <Upload className="w-5 h-5 text-zinc-855 stroke-[2.2]" />
            </button>
            ...
            <button 
              ...
              type="button"
            >
              <Heart className={`w-5 h-5 transition-colors ${isFavorite ? 'fill-rose-500 text-rose-500' : 'text-zinc-700 stroke-[2.2]'}`} />
            </button>
          </div>
        </div>

        {/* Content Pull-Up Container overlaying the image bottom part */}
        <div className="bg-white rounded-t-[36px] -mt-10 relative z-10 pt-7 px-4 sm:px-6 pb-20 text-right">
          ...
          {/* Benefits list section from the screenshot - perfectly matching layout and RTL */}
          <div className="max-w-sm mx-auto px-1 space-y-6 text-right" dir="rtl">
            {/* Benefit: Coffee */}
            <div className="flex items-start gap-4 justify-end">
              <div className="text-right flex-1 select-none">
                ...
                <p className="text-zinc-500 text-[11.5px] font-medium mt-1 leading-relaxed">
                  ابدأ صباحك بفنجان قهوة باستخدام ماكينة صنع القهوة بالتنقيط.
                </p>
              </div>
              <div className="shrink-0 p-0.5">
                <Coffee className="w-6 h-6 text-zinc-800 stroke-[1.8]" />
              </div>
            </div>

            {/* Benefit: Great Communication */}
            <div className="flex items-start gap-4 justify-end">
              <div className="text-right flex-1 select-none">
                <h4 className="font-extrabold text-zinc-900 text-sm">
                  تواصل رائع للمضيف
                ...
                <p className="text-zinc-500 text-[11.5px] font-medium mt-1 leading-relaxed">
                  أحبّ الضيوف الذين قاموا بالحجز مؤخرًا مستوى تواصل {listing.hostName}.
                </p>
              </div>
              <div className="shrink-0 p-0.5">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6 text-zinc-800">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 0 1-2.555-.337A5.972 5.972 0 0 1 5.41 20.97a.75.75 0 0 1-1.074-.765 11.91 11.91 0 0 0 1.8-3.413C4.305 15.344 3 13.791 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25Z" />
                </svg>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-sm mx-auto mt-6 space-y-5">
            ...
            <div className="space-y-4 pt-1 select-none" dir="rtl">
              ...
              <div className="space-y-4 pt-2">
                ...
                {/* Carbon Monoxide Detector - Crossed out line-through text exactly as requested */}
                <div className="flex items-center justify-start gap-3.5">
                  ...
                  <span className="text-zinc-400 font-bold text-[13.5px] text-right line-through">جهاز الكشف عن أول أكسيد الكربون</span>
                </div>

                {/* Smoke Detector - Crossed out line-through text exactly as requested */}
                <div className="flex items-center justify-start gap-3.5">
                  <div className="shrink-0 text-zinc-400">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                  ...
                  <span className="text-zinc-400 font-bold text-[13.5px] text-right line-through">جهاز الكشف عن الدخان</span>
                </div>
              </div>

              {/* Show all 19 amenities button */}
              <button 
                type="button"
        ...
        <div className="fixed sm:absolute bottom-0 left-0 right-0 z-40 max-w-2xl mx-auto w-full flex flex-col pointer-events-none" dir="rtl">
          ...
          <div className="w-full flex flex-col items-center">
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2.5 w-full relative z-10">
                ...
                <div className="flex items-center flex-wrap gap-x-1 sm:gap-x-1.2 justify-start select-none">
                  {["فرصة", "نادرة!", "عادةً", "ما", "يكون", "هذا", "المكان", "محجوزًا"].map((word, wordIndex) => (
                    <motion.span
                      ...
                      className={`inline-block text-[11px] sm:text-[11.5px] font-extrabold tracking-tight transition-colors duration-1000 leading-none ${
                ...
                {/* Iconic Gem representation matching screenshots */}
                <div className="shrink-0 flex items-center justify-center transition-all duration-1000">
                  {bannerState === 'floating' ? (
                    ...
                    /* Flat Outline Diamond Vector from Image 1 */
                    <svg className="w-4 h-4 text-zinc-900 stroke-[2.0]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          ...
          <div className="pointer-events-auto bg-white/95 backdrop-blur-md border-t border-zinc-200 py-4 px-6 flex items-center justify-between shadow-[0_-12px_36px_-15px_rgba(0,0,0,0.12)] rounded-b-none sm:rounded-b-[36px]" dir="rtl">
            ...
            <button
              ...
              onClick={() => setShowBookingReview(true)}
              disabled={isBooked}
              className={`px-8 min-w-[110px] sm:px-11 py-3.5 font-black text-[12.5px] text-white tracking-wide rounded-2xl cursor-pointer haptic-tap transition-all flex items-center justify-center gap-2 ${
                isBooked 
    ...
    >
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur-md z-30 border-b border-zinc-100/90 px-6 py-4 flex items-center justify-start select-none">
        <button 
          onClick={onClose}
          type="button"
          className="text-zinc-900 bg-zinc-50 hover:bg-zinc-100 p-2.5 rounded-full transition-all cursor-pointer haptic-tap flex items-center justify-center border border-zinc-100 shadow-xs"
        >
      ...
      <div className="px-6 py-4 max-w-xl mx-auto w-full space-y-9 select-none">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {/* كؤوس النبيذ */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-800 font-bold text-[13.5px]">كؤوس النبيذ</span>
              <div className="text-zinc-800 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                  <path d="M8 22h8" />
                  <path d="M12 15v7" />
          ...
          {/* SECTION Title: غير مُدرج */}
          <div className="space-y-4 pt-6">
            <h2 className="text-xl sm:text-2xl font-black text-zinc-950 pr-1 mt-5 leading-normal">
              غير مُدرج
            </h2>

            {/* كاميرات المراقبة الخارجية للعقار */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">كاميرات المراقبة الخارجية للعقار</span>
              <div className="text-zinc-450 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
            ...
            {/* تلفزيون */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">تلفزيون</span>
              <div className="text-zinc-450 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
            ...
            {/* غسالة */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">غسالة</span>
              <div className="text-zinc-450 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
            ...
            {/* نشافة */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">نشافة</span>
              <div className="text-zinc-455 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                  <rect x="4" y="4" width="16" height="16" rx="2" />
                  <circle cx="12" cy="12" r="3" />
                  <line x1="1" y1="1" x2="23" y2="23" />
            ...
            {/* مكيف الهواء */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">مكيف الهواء</span>
              <div className="text-zinc-455 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
            ...
            {/* الأساسيات */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">الأساسيات</span>
              <div className="text-zinc-455 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                  <rect x="4" y="4" width="16" height="16" rx="2" />
                  <line x1="1" y1="1" x2="23" y2="23" />
            ...
            <div className="py-4 border-b border-zinc-100/60 text-right">
              <div className="flex items-center justify-between">
                <span className="text-zinc-400 font-bold text-[13.5px] line-through">جهاز الكشف عن الدخان</span>
                <div className="text-zinc-450 shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                    <circle cx="12" cy="12" r="9" />
                    <line x1="1" y1="1" x2="23" y2="23" />
              ...
              <p className="text-zinc-500 text-[11px] font-medium mt-1.5 leading-relaxed">
                قد لا يحتوي هذا المكان على جهاز الكشف عن الدخان. تواصل مع المضيف إذا كان لديك أي أسئلة.
              </p>
            </div>
            ...
            <div className="py-4 border-b border-zinc-100/60 text-right">
              <div className="flex items-center justify-between">
                <span className="text-zinc-400 font-bold text-[13.5px] line-through">جهاز الكشف عن أول أكسيد الكربون</span>
                <div className="text-zinc-405 shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
                    <circle cx="12" cy="12" r="9" />
                    <line x1="1" y1="1" x2="23" y2="23" />
                  </svg>
              ...
              <p className="text-zinc-500 text-[11px] font-medium mt-1.5 leading-relaxed">
                قد لا يحتوي هذا المكان على كاشف أول أكسيد الكربون. تواصل مع المضيف إذا كان لديك أي أسئلة.
              </p>
            </div>

            {/* التدفئة */}
            <div className="flex items-center justify-between py-4 border-b border-zinc-100/60">
              <span className="text-zinc-400 font-bold text-[13.5px] line-through">التدفئة</span>
              <div className="text-zinc-405 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
