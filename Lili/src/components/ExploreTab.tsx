...
export const ExploreTab: React.FC = () => {
    ...
    <div className="flex gap-4 overflow-x-auto pb-4 pt-1 snap-x scrollbar-none">
      {[1, 2, 3].map((index) => (
        <motion.div
          ...
          className="min-w-[210px] max-w-[230px] bg-white rounded-3xl p-3 border border-zinc-100 space-y-3 shadow-xs shrink-0"
        >
          ...
          <div className="space-y-2 px-0.5">
            <div className="flex justify-between items-center">
              ...
              <div className="h-2.5 w-10 rounded-lg skeleton-shimmer" />
            </div>
            <div className="h-4 w-5/6 rounded-lg skeleton-shimmer" />
            <div className="h-2.5 w-1/2 rounded-lg skeleton-shimmer" />
            <div className="h-2.5 w-2/3 rounded-lg skeleton-shimmer" />
          </div>
          {/* Skeleton bottom price row */}
          <div className="flex justify-between items-center pt-1 border-t border-zinc-50">
            <div className="h-3.5 w-16 rounded-lg skeleton-shimmer" />
  ...
  const heroParallaxRef = React.useRef<HTMLDivElement>(null);

  const handleExploreScroll = (e: React.UIEvent<HTMLDivElement>) => {
    const scrollY = (e.target as HTMLDivElement).scrollTop;
    if (heroParallaxRef.current) {
      heroParallaxRef.current.style.transform = `translate3d(0, ${scrollY * 0.3}px, 0)`;
    }
  };

  return (
    <div className="max-w-md mx-auto px-4 sm:px-6 pt-0 pb-6 space-y-5 bg-white text-zinc-900" id="explore-tab-container" dir="rtl">
