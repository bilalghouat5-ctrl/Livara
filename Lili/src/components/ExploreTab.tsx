...
export const ExploreTab: React.FC = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    Haptic.medium(); triggerSkeletonRefresh(() => {
  ...
  const handleWilayaChange = (wilId: string) => {
    Haptic.selection(); triggerSkeletonRefresh(() => setSelectedWilaya(wilId));
  };
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-12">
                {(selectedCategoryForAll === 'find_services_mid' ? randomServices : getFilteredListings(selectedCategoryForAll)).map((listing) => {
                  ...
                  return (
                    ...
                    >
                      {/* Image Container */}
                      <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-zinc-50 border border-zinc-100 shadow-xs mb-3">
                        <motion.img 
                  ...
                  {categoryListings.map((listing) => {
                    ...
                    return (
                      <motion.div 
                        ...
                        key={listing.id}
                        onClick={() => setSelectedListing(listing)}
                        className="group w-[112px] min-w-[112px] sm:w-[124px] sm:min-w-[124px] bg-white snap-start shrink-0 overflow-hidden cursor-pointer flex flex-col justify-between"
