...
import { 
  ...
  ElegantShimmerLoader, 
  ElegantBottomSheet, 
  GlassNotificationBanner, 
  InteractiveMockMap, 
  DraggableInterestBubbles, 
  playHapticSound 
} from './InteractionShowcase';
...
export const ExploreTab: React.FC = () => {
  ...
  // 1. Toast Notification States
  const [toasts, setToasts] = useState<{ id: string; title: string; message: string; type: 'info' | 'success' | 'warn' }[]>([]);
  const addToast = (title: string, message: string, type: 'info' | 'success' | 'warn' = 'info') => {
  ...
  // 2. Interactive Map Overlay States
  const [isMapOpen, setIsMapOpen] = useState(false);
  const [showRealMap, setShowRealMap] = useState(true);

  // 3. Elegant Bottom Sheet States
  const [isBottomSheetOpen, setIsBottomSheetOpen] = useState(false);
  const [userInterests, setUserInterests] = useState<string[]>(['إطلالة بحرية', 'مسبح خاص', 'تكييف كامل', 'حديقة واسعة', 'ألعاب أطفال']);

  // 4. Infinite Scroll state tracking page size limit
  const [visibleCount, setVisibleCount] = useState(8);
  const [isInfiniteLoading, setIsInfiniteLoading] = useState(false);

  // 5. Pull-To-Refresh simulate handler
  const handleFeedRefresh = async () => {
    addToast('تحديث فوري', 'جاري جلب أحدث الإعلانات والأسعار لقائمتك...', 'info');
    setIsLoading(true);
    await new Promise(resolve => setTimeout(resolve, 800));
    setIsLoading(false);
    addToast('تم التحديث بنجاح', 'لقد تم تحديث ومزامنة كامل العقارات وعروض الجزائر الحالية.', 'success');
  };

  useEffect(() => {
    const handleScroll = (e: Event) => {
      const target = e.target as HTMLElement;
      const isWindow = target === (document as any) || (target as any) === window;
      const scrollTop = isWindow ? window.scrollY : (target.scrollTop || 0);
      ...
      // Check for Infinite Scroll bottom reach trigger
      ...
      if (scrollHeight - scrollTop - clientHeight < 120 && !isInfiniteLoading && visibleCount < listings.length) {
        setIsInfiniteLoading(true);
        setTimeout(() => {
          setVisibleCount(prev => Math.min(listings.length, prev + 4));
          setIsInfiniteLoading(false);
          addToast('تم تحميل عروض إضافية', 'تلقائياً، تم عرض خيارات جديدة متوافقة مع اهتماماتك.', 'info');
        }, 600);
      }
    };
  ...
  }, [isInfiniteLoading, visibleCount, listings.length]);
  ...
  return (
    <PullToRefreshContainer onRefresh={handleFeedRefresh}>
          ...
          <div className="space-y-6 animate-fade-in" dir="rtl">
            ...
            {(selectedCategoryForAll === 'find_services_mid' ? randomServices : getFilteredListings(selectedCategoryForAll)).length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-12">
                {(selectedCategoryForAll === 'find_services_mid' ? randomServices : getFilteredListings(selectedCategoryForAll)).slice(0, visibleCount).map((listing) => {
            ...
            {/* Elegant Shimmer Loader for Infinite Scroll */}
            {((selectedCategoryForAll === 'find_services_mid' ? randomServices : getFilteredListings(selectedCategoryForAll)).length > visibleCount) && (
              <div className="py-6 flex flex-col items-center justify-center gap-2">
                <ElegantShimmerLoader rows={1} />
                <p className="text-[10px] font-black text-zinc-400">جاري تحميل المزيد من العقارات والمميزات...</p>
              </div>
            )}
          </div>
        ) : activeMainTab === 'experiences' ? (
      ...
      {/* 2.1. REAL-TIME TOAST NOTIFICATION LAYER */}
      <GlassNotificationBanner toasts={toasts} removeToast={removeToast} />

      {/* 2.2. PREMIUM BENTO CAPSULE FLOATING ACTION BUTTON (FAB) */}
      <div className="fixed bottom-22 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-zinc-900/95 hover:bg-zinc-950 text-white backdrop-blur-md px-5 py-2.5 rounded-full shadow-[0_4px_20px_rgba(0,0,0,0.18)] border border-white/10 transition-all duration-300 active:scale-95 cursor-pointer">
        <button 
          onClick={() => {
            playHapticSound('success');
            setIsMapOpen(true);
          }}
          className="flex items-center gap-1.5 text-xs font-black border-l border-white/10 pl-3 cursor-pointer"
        >
        ...
        <button 
          ...
          className="flex items-center gap-1.5 text-xs font-black pr-1 cursor-pointer"
        >
      ...
      {/* 2.3. INTERACTIVE REAL GOOGLE MAPS VS. SIMULATOR FALLBACK */}
      <AnimatePresence>
        {isMapOpen && (
          showRealMap ? (
            <InteractiveGoogleMap
              ...
              onSelectListing={(listing) => {
                ...
                addToast('تفاصيل العقار', `تم فتح ${listing.title[language]} من الخريطة`, 'success');
              }}
              onShowMockMap={() => {
            ...
            <InteractiveMockMap 
              ...
              onClose={() => {
                setIsMapOpen(false);
                // Reset real map mode for next map interaction
                setShowRealMap(true);
              }}
              onSelectListing={(listing) => {
                playHapticSound('select');
      ...
      {/* 2.4. DRAGGABLE INTEREST BOTTOM SHEET */}
      <ElegantBottomSheet 
        isOpen={isBottomSheetOpen} 
        onClose={() => setIsBottomSheetOpen(false)} 
        title="تفضيلات السكن الجزائرية"
      >
        <div className="space-y-4 py-2" dir="rtl">
          <p className="text-[11px] font-bold text-zinc-400 leading-relaxed text-right">
            اسحب وأعد ترتيب تفضيلاتك وسوف نقوم بإعادة ترتيب العروض والخدمات لك مباشرة!
          </p>
          <DraggableInterestBubbles 
            tags={userInterests} 
            onChange={(newTags) => {
              setUserInterests(newTags);
              playHapticSound('success');
              addToast('تحديث الترتيب', 'تم تحديث خوارزمية ترتيب العقارات بناءً على تفضيلاتك.', 'success');
            }} 
          />
          <div className="pt-2 flex justify-end">
            <button 
              onClick={() => {
                ...
                addToast('العروض المخصصة جاهزة', 'تم تفعيل الترتيب المفضل بنجاح.', 'success');
              }}
              className="w-full py-3 bg-zinc-950 text-white font-black text-xs rounded-full hover:bg-zinc-855 transition active:scale-95 cursor-pointer text-center"
            >
              حفظ وتطبيق الخوارزمية الذكية ✨
            </button>
          </div>
        </div>
      </ElegantBottomSheet>
