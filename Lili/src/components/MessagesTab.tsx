...
export const MessagesTab: React.FC<MessagesTabProps> = ({ onLoginRequested }) => {
  const { messages, sendMessageToHost, isRegistered } = useApp();
  const { t, language } = useLanguage();

  if (!isRegistered) {
    return (
      <div className="max-w-md mx-auto px-6 pt-16 pb-12 space-y-7 bg-white min-h-[85vh] text-right font-sans flex flex-col justify-start animate-fade-in" id="messages-tab-unregistered" dir="rtl">
        {/* Title */}
        ...
        <div className="space-y-2 pt-4">
          <h2 className="text-[17px] font-extrabold text-zinc-900 leading-snug">
            سجّل الدخول لرؤية الرسائل
          </h2>
          <p className="text-zinc-500 font-bold text-xs leading-relaxed max-w-[325px]">
            عندما تسجل الدخول، ستجد الرسائل الواردة من المضيفين هنا.
          </p>
        </div>
