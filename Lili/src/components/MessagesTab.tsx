...
export const MessagesTab: React.FC = () => {
  ...
  return (
    <div className="w-full h-full bg-white text-zinc-900" id="messages-tab-container" dir="rtl">
      
      {activeMessage ? (
        /* ==================== ACTIVE CHAT WORKSPACE (SUB-VIEW) ==================== */
        <SwipeBackWrapper onSwipeBack={() => { setActiveMessageId(null); }}>
        <div className="space-y-4 animate-fade-in flex flex-col h-full bg-white px-4 py-2" dir="rtl">
          {/* Header Bar */}
          <div className="flex items-center justify-between pb-3 border-b border-zinc-100/90">
            <div className="flex items-center gap-3 text-right">
              ...
              <button
                onClick={() => { Haptic.light(); setActiveMessageId(null)}}
            ...
            <button 
              ...
              className="w-9 h-9 bg-zinc-100 hover:bg-zinc-200 rounded-full flex items-center justify-center transition-all cursor-pointer active:scale-95 text-zinc-850"
              title="اتصال تلفني"
            >
          ...
          <AnimatePresence>
            {copiedPhoneHost && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="bg-zinc-900 text-white p-3 rounded-2xl border border-zinc-800 shadow-lg text-[11px] font-bold text-center flex items-center justify-between"
              >
                <span>رقم المضيف للتنسيق: <b className="text-rose-400 font-mono tracking-wider">0552 44 91 30</b></span>
                <span className="text-[9px] bg-zinc-800 text-zinc-300 py-0.5 px-2 rounded-full">جاهز للاتصال</span>
              </motion.div>
            )}
          </AnimatePresence>

          {/* Chat Bubbles List */}
          <div className="flex-1 space-y-3.5 py-4 overflow-y-auto max-h-[380px] no-scrollbar flex flex-col gap-1 pr-1 pl-1">
                ...
                >
                  <div className={`max-w-[80%] rounded-[1.25rem] px-4 py-2.5 shadow-xs ${
                    isUser 
                      ...
                      : 'bg-zinc-100 text-zinc-950 rounded-bl-none text-right'
                  }`}>
                    <p className="text-[12px] font-medium leading-relaxed whitespace-pre-line text-right">{msg.text}</p>
            ...
            <div className="space-y-2.5 pt-1">
              ...
              <div className="space-y-2">
                {filteredMessages.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => { Haptic.light(); setActiveMessageId(item.id)}}
