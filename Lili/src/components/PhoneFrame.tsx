...
export const PhoneFrame: React.FC<PhoneFrameProps> = ({ children }) => {
  ...
  // Auto-switch to full on narrow screens
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-slate-50 text-zinc-900 transition-all duration-500 flex flex-col md:flex-row justify-center items-center p-0 md:p-6 lg:p-8 overflow-x-hidden select-none">
      ...
      <div className={`relative flex flex-col items-center justify-center transition-all duration-500 ${
        ...
        <div
          className={`relative transition-all duration-500 flex flex-col overflow-hidden bg-white ${
            viewMode === 'phone'
              ? 'w-full h-[100dvh] sm:h-[840px] rounded-none sm:rounded-[48px] border-0 sm:border-[11px] sm:border-zinc-950 shadow-none sm:shadow-[0_25px_60px_rgba(0,0,0,0.18)] sm:ring-1 sm:ring-zinc-800/40'
              : 'w-full h-[100dvh] rounded-none'
          }`}
          style={{ fontSize: viewMode === 'phone' ? '14px' : '16px', filter: `brightness(${brightness}%)` }}
        >
          {/* Status bar */}
          {viewMode === 'phone' && (
